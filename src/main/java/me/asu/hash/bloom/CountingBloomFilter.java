 ...
 * @see <a href="http://portal.acm.org/citation.cfm?id=343571.343572">Summary cache: a scalable
 * wide-area web cache sharing protocol</a>
 */
public final class CountingBloomFilter extends Filter {
    ...
    public void add(Key key) {
        ...
        for (int i = 0; i < nbHash; i++) {
            ...
            if (bucketValue < BUCKET_MAX_VALUE) {
                // increment by 1
                buckets[wordNum] =
                        (buckets[wordNum] & ~bucketMask) | ((bucketValue + 1) << bucketShift);
            }
        }
    }

    /**
     * Removes a specified key from <i>this</i> counting Bloom filter. <p> <b>Invariant</b>: nothing
     * happens if the specified key does not belong to <i>this</i> counter Bloom filter.
     *
     ...
     * @return 0 if the key is not present. Otherwise, a positive value v will be returned such that
     * <code>v == count</code> with probability equal to the error rate of this filter, and <code>v
     * &gt; count</code> otherwise. Additionally, if the filter experienced an underflow as a result
     * of {@link #delete(Key)} operation, the return value may be lower than the <code>count</code>
     * with the probability of the false negative rate of such filter.
     */
