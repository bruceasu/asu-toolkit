...
public class DataChecksum implements Checksum {
    ...
    public static enum Type {
        NULL(CHECKSUM_NULL, 0), CRC32(CHECKSUM_CRC32, 4), CRC32C(CHECKSUM_CRC32C, 4), DEFAULT(
                CHECKSUM_DEFAULT, 0), // This cannot be used to create DataChecksum
        ...
        public static Type valueOf(int id) {
            if (id < 0 || id >= values().length) {
                throw new IllegalArgumentException(
                        "id=" + id + " out of range [0, " + values().length + ")");
            }
            return values()[id];
        }
    }
    ...
    public static Checksum newCrc32() {
        return isJava7OrAbove() ? new CRC32() : new PureJavaCrc32();
    }
    ...
    public static DataChecksum newDataChecksum(Type type, int bytesPerChecksum) {
        ...
        switch (type) {
            ...
            default:
                return new DataChecksum(type, new Adler32(), bytesPerChecksum);
        }
    }
    ...
    public static DataChecksum newDataChecksum(byte bytes[], int offset) {
        ...
        int bytesPerChecksum =
                ((bytes[offset + 1] & 0xff) << 24) | ((bytes[offset + 2] & 0xff) << 16) | (
    ...
    public static DataChecksum newDataChecksum(DataInputStream in) throws IOException {
        ...
        if (summer == null) {
            throw new IOException(
                    "Could not create DataChecksum of type " + type + " with bytesPerChecksum "
                            + bpc);
        }
        return summer;
    }
    ...
    public boolean compare(byte buf[], int offset) {
        if (type.size == 4) {
            int checksum = ((buf[offset + 0] & 0xff) << 24) | ((buf[offset + 1] & 0xff) << 16) | (
     ...
     * @throws Exception if the checksums do not match
     */
    public void verifyChunkedSums(ByteBuffer data, ByteBuffer checksums, String fileName,
                                  long basePos) throws Exception {
        if (type.size == 0) {
            return;
        }

        if (data.hasArray() && checksums.hasArray()) {
            verifyChunkedSums(data.array(), data.arrayOffset() + data.position(), data.remaining(),
                    checksums.array(), checksums.arrayOffset() + checksums.position(), fileName,
                    basePos);
            return;
        }
        ...
        try {
            ...
            while (data.remaining() > 0) {
                ...
                int stored = (sum[0] << 24 & 0xff000000) | (sum[1] << 16 & 0xff0000) | (sum[2] << 8
                        & 0xff00) | sum[3] & 0xff;
                if (calculated != stored) {
                    long errPos = basePos + data.position() - startDataPos - n;
                    throw new Exception(
                            "Checksum error: " + fileName + " at " + errPos + " exp: " + stored
                                    + " got: " + calculated);
                }
            }
        } finally {
    ...
    throws Exception {
        ...
        while (remaining > 0) {
            ...
            int stored = (checksums[checksumsOff] << 24 & 0xff000000) | (
                    ...
                    checksums[checksumsOff + 2] << 8 & 0xff00) | checksums[checksumsOff + 3] & 0xff;
            checksumsOff += 4;
            if (calculated != stored) {
     ...
     * @param checksums the DirectByteBuffer into which checksums will be stored. Enough space must
     *                  be available in this buffer to put the checksums.
     */
    public void calculateChunkedSums(ByteBuffer data, ByteBuffer checksums) {
        ...
        if (data.hasArray() && checksums.hasArray()) {
            calculateChunkedSums(data.array(), data.arrayOffset() + data.position(),
                    data.remaining(), checksums.array(),
                    checksums.arrayOffset() + checksums.position());
            return;
        }
     ...
     * is to avoid the copy when dealing with ByteBuffers that have array backing.
     */
    public void calculateChunkedSums(byte[] data, int dataOffset, int dataLength, byte[] sums,
                                     int sumsOffset) {
        if (type.size == 0) {
            return;
        }
