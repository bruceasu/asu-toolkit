...
/**
 * 汉语拼音工具类 （GB2312）
 * Mar 2, 2009 5:02:29 PM
 */
public class PinyinUtil {

  private static Map<String, Integer> spellMap = new LinkedHashMap<String, Integer>(450);
  private static Map<Character, String> specialCharacter2SpellMap = new HashMap<Character, String>();

  static {
    initialize();
    registerSpecialCharacterSpell('琪', "qi");
  }
   ...
   * @param pinyin
   */
  public static void registerSpecialCharacterSpell(char character, String pinyin) {
    specialCharacter2SpellMap.put(character, pinyin);
  }
  ...
  public static String getPinyin(String cnStr) {
    ...
    for (int i = 0, Len = chars.length; i < Len; i++) {
      ...
      if (ascii == 0) { //取ascii时出错
        ...
        if (spell == null) {
          //特殊字符的处理
          if (specialCharacter2SpellMap.containsKey(chars[i])) {
            retuBuf.append(specialCharacter2SpellMap.get(chars[i]));
          } else {
            retuBuf.append(chars[i]);
          }
        } else {
