package me.asu.cache.impl;

import java.util.HashMap;
import java.util.Iterator;
import me.asu.cache.CacheObj;
	...
	protected int pruneCache() {
		...
		while (values.hasNext()) {
			...
			if (co.isExpired() == true) {
				values.remove();
				onRemove(co.getKey(), co.getObj());
				count++;
				continue;
			}

			//找出访问最少的对象
			if (comin == null || co.getAccessCount() < comin.getAccessCount()) {
				comin = co;
			}
		}
		...
		if (isFull() && comin != null) {
			...
			while (values.hasNext()) {
				co1 = values.next();
				co1.setAccessCount(co1.getAccessCount() - minAccessCount);
				if (co1.getAccessCount() <= 0) {
					values.remove();
					onRemove(co1.getKey(), co1.getObj());
					count++;
				}
			}
		}
