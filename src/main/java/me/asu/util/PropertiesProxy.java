...
import io.spring.ResourcePatternResolver;
    ...
    private List<IResource> getResources(String... paths) {
        ...
        for (String path : paths) {
            try {
                ...
                IResource[]             resources        = resourceResolver.getResources(
                        "^.+[.]properties$");
                ...
                if (ignoreResourceNotFound) {
                    if (log.isWarnEnabled()) {
                        log.warn("Could not load resource from " + path + ": "
                                         + e.getMessage());
                    }
                } else {
                    throw Exceptions.wrapThrow(e);
    ...
    public static class MultiLineProperties implements Map<String, String> {
        ...
        throws IOException {
            ...
            while (null != (s = tr.readLine())) {
                ...
                if (s.length() > 0
                        && s.trim().charAt(0) == '#') // 只要第一个非空白字符是#,就认为是注释
                {
                    continue;
                }
                int  pos;
