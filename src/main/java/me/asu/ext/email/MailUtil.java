...
public class MailUtil {
    ...
    public static boolean checkSmtpServerIsReady() {
        return Strings.isNotEmpty(SMTPServer)
                && Strings.isNotEmpty(SMTPUsername)
    ...
    private static void loadConfigProperties() throws FileNotFoundException {
        ...
        try {
            ...
            LOGGER.log(Level.SEVERE,
                    "File not found at " + f.getAbsolutePath(), e);
        } catch (IOException e) {
            LOGGER.log(Level.SEVERE,
                    "Error reading config file " + f.getName(), e);
        }
    ...
    public static boolean sendAnonymousEmail(MailMessage mail) {
        ...
        try {
            ...
            for (String to : tos) {
                ...
                Attributes attrs = ctx.getAttributes(domain, new String[]{
                        "MX"});
                if (attrs == null || attrs.size() <= 0) {
                    throw new IllegalStateException(
                            "Error: Your DNS server has no Mail eXchange records!");
                }
                @SuppressWarnings("rawtypes") NamingEnumeration servers  = attrs
                        .getAll();
                ...
                while (servers.hasMore()) {
                    ...
                    for (int i = 0; i < hosts.size(); ++i) {
                        ...
                        smtpHost = smtpHost.substring(
                                smtpHost.lastIndexOf(' ') + 1);
                        try {
                ...
                if (!isSend) {
                    throw new IllegalStateException(
                            "Error: Send email error." + buf.toString());
                }
            }
        } catch (Throwable e) {
            LOGGER.log(Level.SEVERE, "", e);
    ...
    private static boolean sendEmail(String smtpHost,
        ...
        return sendEmail(smtpHost, mail.getSubject(), mail.getFrom(), mail.getTos(), mail
                        .getCcs(),
     ...
     */
    public static boolean receiveEmail(String host,
            String username,
            String password) {
    ...
    private static String mimeDecodeString(String res) {
        if (res != null) {
            ...
            try {
                ...
                LOGGER.log(Level.SEVERE,
                        "Decode string error. Origin string is: " + res, e);
            }
            return from;
        }
        return null;
