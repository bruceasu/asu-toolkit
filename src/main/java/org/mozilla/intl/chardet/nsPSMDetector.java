...
public abstract class nsPSMDetector {
	...
	public nsPSMDetector(final int langFlag) {
		initVerifiers(langFlag);
		Reset();
	}

	public nsPSMDetector(final int aItems, final nsVerifier[] aVerifierSet,
			final nsEUCStatistics[] aStatisticsSet) {
	...
	public boolean HandleData(final byte[] aBuf, final int len) {
	...
	public abstract void Report(final String charset);
	...
	public void Sample(final byte[] aBuf, final int aLen) {
		Sample(aBuf, aLen, false);
	}

	public void Sample(final byte[] aBuf, final int aLen, final boolean aLastChance) {
	...
	protected void initVerifiers(final int currVerSet) {
		...
		if (currVerifierSet == nsPSMDetector.TRADITIONAL_CHINESE) {
			...
			mVerifier = new nsVerifier[] { new nsUTF8Verifier(), new nsBIG5Verifier(),
					new nsISO2022CNVerifier(), new nsEUCTWVerifier(),
		...
		// ==========================================================
			...
			mVerifier = new nsVerifier[] { new nsUTF8Verifier(), new nsGB2312Verifier(),
					...
					new nsHZVerifier(), new nsEUCTWVerifier(),
					// new nsCP1252Verifier(),
					new nsUCS2BEVerifier(), new nsUCS2LEVerifier() };
