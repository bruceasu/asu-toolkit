...
public abstract class ImageUtils {
     ...
     * @return 旋转后得图像对象
     */
    public static BufferedImage rotate(String srcPath,
                                       String taPath,
                                       int degree) throws IOException {
        File srcIm = new File(srcPath);
        if (null == srcIm) {
            throw Exceptions.makeThrow("Fail to find image file '%s'!",
                                       srcPath);
        }
    ...
    public static BufferedImage clipScale(Object srcIm,
        ...
        BufferedImage old = read(srcIm);
        BufferedImage im = ImageUtils.clipScale(old.getSubimage(startPoint[0],
                                                                startPoint[1],
                                                                width,
                                                                height),
                                                width,
    ...
    private static BufferedImage createJPEG4(Raster raster) {
        ...
        for (int i = 0, imax = Y.length, base = 0; i < imax; i++, base += 3) {
            float k = 220 - K[i], y = 255 - Y[i], cb = 255 - Cb[i], cr =
                    255 - Cr[i];
    ...
    public static String encodeBase64(File targetFile) {
        ...
        return encoder.encodeToString(bImage);
    }
    ...
    public static void showSupportedFormat() {
        System.out.println("支持写的图片格式:"
                                   + java.util.Arrays.toString(ImageIO.getWriterFormatNames()));
        System.out.println("支持读的图片格式:"
     ...
     */
    public static BufferedImage readImg(String srcImageFile)
    throws IOException {
     ...
     */
    public final static BufferedImage scale(BufferedImage src,
                                            int scale,
                                            boolean flag) {
        // 得到源图宽
    ...
    public final static void scale(String srcImageFile,
                                   ...
                                   boolean bb) {
        try {
            BufferedImage itemp = scale(readImg(srcImageFile),
                                        width,
                                        height,
                                        bb);
     ...
     * @return {@link BufferedImage}
     */
    public final static BufferedImage scale(BufferedImage bi,
                                            int width,
                                            int height,
                                            boolean bb) {
        // 缩放比例
        double ratio = 0.0;
        Image itemp = bi.getScaledInstance(width,
                                           height,
     ...
     */
    public final static void cutWithFixedSize(String srcImageFile,
                                              String descDir,
                                              int destWidth,
                                              int destHeight) {
        try {
     ...
     */
    public final static BufferedImage[][] cutWithFixedSize(BufferedImage bi,
                                                           int width,
                                                           int height) {
        ...
        if (srcWidth > width && srcHeight > height) {
            ...
            for (int i = 0; i < rows; i++) {
                for (int j = 0; j < cols; j++) {
                    // 四个参数分别为图像起点坐标和宽高
                    // 即: CropImageFilter(int x,int y,int width,int height)
                    cropFilter = new CropImageFilter(j * width,
                                                     i * height,
                                                     width,
                                                     height);
                    img        = Toolkit.getDefaultToolkit()
     ...
     */
    public static boolean createThumbnail(String src,
                                          String dist,
                                          float width,
                                          float height,
                                          boolean flag) {
        ...
        try {
            ...
            int newWidth =
                    flag ? (int) width : (int) (image.getWidth() * ratio);
            int newHeight =
                    flag ? (int) height : (int) (image.getHeight() * ratio);
            BufferedImage bfImage = new BufferedImage(newWidth,
                                                      newHeight,
                                                      BufferedImage.TYPE_INT_RGB);
            flag1 = bfImage.getGraphics()
                           .drawImage(image.getScaledInstance(newWidth,
                                                              newHeight,
                                                              Image.SCALE_SMOOTH),
                                      0,
                                      0,
    ...
    public static final class FingerPrint {
        ...
        private static BufferedImage resize(Image src, int width, int height) {
            ...
            try {
                g.drawImage(src.getScaledInstance(width,
                                                  height,
                                                  Image.SCALE_SMOOTH),
                            0,
                            0,
        ...
        public boolean equals(Object obj) {
            if (obj instanceof FingerPrint) {
                return me.asu.util.Arrays.equals(this.binaryzationMatrix,
        ...
        public float compare(FingerPrint src) {
            if (src.binaryzationMatrix.length
        ...
        public static float compare(BufferedImage image1,
