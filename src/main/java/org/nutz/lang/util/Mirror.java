...
public class Mirror<T> {
    ...
    public static <T> Mirror<T> me(Class<T> classOfT, TypeExtractor typeExtractor) {
        return null == classOfT ? null
                                : new Mirror<T>(classOfT)
                       .setTypeExtractor(typeExtractor == null ? defaultTypeExtractor
                                                               : typeExtractor);
    }
    ...
    public static void evalGetterSetter(Method method,
        ...
        // 是 setter
            ...
            // 寻找 getter
            try {
                getter = method.getDeclaringClass().getMethod("get" + Strings
                        .upperFirst(name));
            } catch (Exception e) {
            }
    ...
    private static RuntimeException makeSetValueException(Class<?> type,
        ...
        return new FailToSetValueException(
                String.format("Fail to set value [%s] to [%s]->[%s] because '%s'",
                              value,
                              type.getName(),
                              name,
                              e.getMessage()),
    ...
    public Field getField(String name) throws NoSuchFieldException {
        ...
        throw new NoSuchFieldException(
                String.format("Can NOT find field [%s] in class [%s] and it's parents classes",
                              name,
                              klass.getName()));
    }
