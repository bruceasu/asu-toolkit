...
import static org.nutz.lang.util.Mirror.*;
...
public abstract class Borns {
	...
	private static <T> BornContext<T> evalWithArgs(Class<T> type, Object[] args) {
		// 准备变参数组
		Object dynaArg = evalArgToSameTypeRealArray(args);

		// 准备好参数类型
		Class<?>[] argTypes = evalToTypes(args);
	...
	private static <T> BornContext<T> evalWithArgTypes(boolean accurate,
		...
		for (Constructor<?> cc : type.getConstructors()) {
			...
			MatchType mt = matchParamTypes(pts, argTypes);
			re.setMatchType(mt);
			// 正好合适
			if (MatchType.YES == mt) {
			...
			// 差一个参数，说明这个构造函数有变参数组
				...
				re.setLackArg(blankArrayArg(pts));
				return re.setBorning(new ConstructorBorning(cc));
			}
			// 看看整个输入的参数是不是变参
