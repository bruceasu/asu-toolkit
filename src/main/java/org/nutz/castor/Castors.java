...
public class Castors {
     ...
     * 设置转换的配置
     * <p>
     * 配置对象所有的共有方法都会被遍历。只要这个方法有一个且只有一个参数，并且该参数 是一个 org.nutz.cast.Castor
        ...
        for (Class<?> klass : classes) {
            try {
                ...
                if (log.isWarnEnabled()) {
                    log.warn("Fail to create cast [{}] because: {}", klass, e.getMessage());
                }
            }
        }
        if (log.isDebugEnabled()) {
            log.debug("Using {} cast for Castors", map.size());
        }
    }
    ...
    throws FailToCastObjectException {
        ...
        if (null != componentType && fromType != String.class && componentType
        ...
        if (null == c) {
            throw new FailToCastObjectException(
                    String.format("Can not find cast for '%s'=>'%s' in (%d) because:\n%s",
                            fromType.getName(), toType.getName(), map.size(),
                            "Fail to find matched cast"));
        }
        if (Object2Object.class.getName().equals(c.getClass().getName()) && from
                .canCastToDirectly(toType)) { // Use language built-in cases
            return (T) src;
        }
        try {
            ...
            throw new FailToCastObjectException(
                    String.format("Fail to cast from <%s> to <%s> for {%s} because:\n%s:%s",
                            fromType.getName(), toType.getName(), src, e.getClass().getSimpleName(),
                            e.getMessage()), Lang.unwrapThrow(e));
        }
    }
