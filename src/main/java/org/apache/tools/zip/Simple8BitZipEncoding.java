...
class Simple8BitZipEncoding implements ZipEncoding {
	...
	private static final class Simple8BitChar implements Comparable<Simple8BitChar> {
		...
		public String toString() {
			return "0x" + Integer.toHexString(0xffff & unicode) + "->0x"
					+ Integer.toHexString(0xff & code);
		}
	}
	...
	public Simple8BitZipEncoding(char[] highChars) {
		...
		reverseMapping = Collections.unmodifiableList(temp);
	}
	...
	private Simple8BitChar encodeHighChar(char c) {
		...
		// binary search...
		int i0 = 0;
		int i1 = reverseMapping.size();

		while (i1 > i0) {

			int i = i0 + (i1 - i0) / 2;

			Simple8BitChar m = reverseMapping.get(i);
		...
		if (i0 >= reverseMapping.size())
			return null;

		Simple8BitChar r = reverseMapping.get(i0);
