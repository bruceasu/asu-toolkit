 ...
 * RandomAccessFile} when you know that the output is going to go to a file.
 * </p>
 * 
 * <p>
...
public class ZipOutputStream extends FilterOutputStream {
	...
	public static final class UnicodeExtraFieldPolicy {
		 ...
		 */
		public static final UnicodeExtraFieldPolicy NOT_ENCODEABLE = new UnicodeExtraFieldPolicy(
				"not encodeable");
	 ...
	 *             compressed size exceeds 4 GByte and {@link #setUseZip64} is
	 ...
	 * @throws Zip64RequiredException
	 *             if the entry's uncompressed or
	 *             compressed size is known to exceed 4 GByte and
	 ...
	 * AsNeeded}, Zip64 extensions will transparently be used for those entries
	 * that require them. This mode can only be used if the uncompressed size of
	 * the {@link ZipEntry} is known when calling {@link #putNextEntry} or the
	 * archive is written to a seekable output (i.e. you have used the
	 * {@link #ZipOutputStream(File) File-arg constructor}) - this mode
	...
	private void addUnicodeExtraFields(ZipEntry ze, boolean encodable, ByteBuffer name)
		...
		if (createUnicodeExtraFields == UnicodeExtraFieldPolicy.ALWAYS || !encodable) {
			ze.addExtraField(new UnicodePathExtraField(ze.getName(), name.array(), name
					.arrayOffset(), name.limit() - name.position()));
		}
		...
		if (comm != null && !"".equals(comm)) {
			...
			if (createUnicodeExtraFields == UnicodeExtraFieldPolicy.ALWAYS || !commentEncodable) {
				...
				ze.addExtraField(new UnicodeCommentExtraField(comm, commentB.array(), commentB
						.arrayOffset(), commentB.limit() - commentB.position()));
			}
		}
	}
		...
		if (entry.entry.getMethod() == DEFLATED) {
			...
			if (entry.entry.getCrc() != crc)
				throw new ZipException("bad CRC checksum for entry " + entry.entry.getName() + ": "
						+ Long.toHexString(entry.entry.getCrc()) + " instead of "
						+ Long.toHexString(crc));

			if (entry.entry.getSize() != bytesWritten)
				throw new ZipException("bad size for entry " + entry.entry.getName() + ": "
						+ entry.entry.getSize() + " instead of " + bytesWritten);
	...
	private void rewriteSizesAndCrc(boolean actuallyNeedsZip64) throws IOException {
		...
		if (hasZip64Extra(entry.entry)) {
			...
			raf.seek(entry.localDataStart + 3 * WORD + 2 * SHORT + getName(entry.entry).limit() + 2
					* SHORT);
	...
	private boolean shouldAddZip64Extra(ZipEntry entry, Zip64Mode mode) {
		return mode == Zip64Mode.Always || entry.getSize() >= ZIP64_MAGIC
	...
	private void writeVersionNeededToExtractAndGeneralPurposeBits(final int zipMethod,
			final boolean utfFallback, final boolean zip64) throws IOException {
	 ...
	 * @throws Zip64RequiredException
	 *             if the archive's size exceeds 4
	 *             GByte and {@link Zip64Mode #setUseZip64} is
	 *             {@link Zip64Mode#Never}.
	 */
	protected void writeCentralFileHeader(ZipEntry ze) throws IOException {
		...
		final boolean needsZip64Extra = hasZip64Extra(ze) || ze.getCompressedSize() >= ZIP64_MAGIC
				|| ze.getSize() >= ZIP64_MAGIC || lfhOffset >= ZIP64_MAGIC;
