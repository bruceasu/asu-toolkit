 ...
 * entries. As of Apache Ant 1.9.0 it also transparently supports Zip64
...
public class ZipFile {
	 ...
	 */
	private final Map<String, LinkedList<ZipEntry>> nameMap = new HashMap<String, LinkedList<ZipEntry>>(
			HASH_SIZE);
	 ...
	 * @param name
	 *            name of the entry.
	 * @return the ZipEntry corresponding to the given name - or {@code null} if
	 *         not present.
	 */
	 ...
	 *            added to this map.
	 */
	private void readCentralDirectoryEntry(Map<ZipEntry, NameAndComment> noUTF8Flag)
			throws IOException {
	 ...
	 * size or neither of both as the internal logic in ExtraFieldUtils forces
	 * the field to create local header data even if they are never used - and
	 * here a field with only one size would be invalid.
	 * </p>
	 */
	private void setSizesAndOffsetFromZip64Extra(ZipEntry ze, OffsetEntry offset, int diskStart)
			throws IOException {
		...
		if (z64 != null) {
			...
			z64.reparseCentralDirectoryData(hasUncompressedSize, hasCompressedSize,
					hasRelativeHeaderOffset, diskStart == ZIP64_MAGIC_SHORT);
	 ...
	 */
	private boolean tryToLocateSignature(long minDistanceFromEnd, long maxDistanceFromEnd,
