...
public class TarInputStream extends FilterInputStream {
	...
	public TarEntry getNextEntry() throws IOException {
		...
		if (currEntry != null) {
			...
			if (debug) {
				System.err.println("TarInputStream: SKIP currENTRY '" + currEntry.getName()
						+ "' SZ " + entrySize + " OFF " + entryOffset + "  skipping " + numToSkip
						+ " bytes");
			}
	...
	public long skip(long numToSkip) throws IOException {
		...
		long skip = numToSkip;
		while (skip > 0) {
			int realSkip = (int) (skip > SKIP_BUF.length ? SKIP_BUF.length : skip);
	...
	private void applyPaxHeadersToCurrentEntry(Map<String, String> headers) {
		 ...
		 * atime, ctime, charset: cannot use these without changing TarEntry
		 * fields
		 * mtime
		 * comment
		 * gid, gname
