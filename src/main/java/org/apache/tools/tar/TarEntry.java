...
public class TarEntry implements TarConstants {
	 ...
	 */
	private static boolean isEqual(final byte[] buffer1, final int offset1, final int length1,
			final byte[] buffer2, final int offset2, final int length2, boolean ignoreTrailingNulls) {
	...
	public TarEntry(File file, String fileName) {
		...
		modTime = file.lastModified() / MILLIS_PER_SECOND;
		devMajor = 0;
		devMinor = 0;
	}
	...
	public TarEntry(String name, boolean preserveLeadingSlashes) {
		...
		linkFlag = isDir ? LF_DIR : LF_NORMAL;
		userId = 0;
		groupId = 0;
		size = 0;
		modTime = (new Date()).getTime() / MILLIS_PER_SECOND;
		linkName = "";
		userName = "";
		groupName = "";
	}
	...
	private void parseTarHeader(byte[] header, ZipEncoding encoding, final boolean oldStyle)
		...
		name = oldStyle ? TarUtils.parseName(header, offset, NAMELEN) : TarUtils.parseName(header,
		...
		linkName = oldStyle ? TarUtils.parseName(header, offset, NAMELEN) : TarUtils.parseName(
				header, offset, NAMELEN, encoding);
		offset += NAMELEN;
		magic = TarUtils.parseName(header, offset, PURE_MAGICLEN);
		offset += PURE_MAGICLEN;
		version = TarUtils.parseName(header, offset, VERSIONLEN);
		offset += VERSIONLEN;
		userName = oldStyle ? TarUtils.parseName(header, offset, UNAMELEN) : TarUtils.parseName(
				header, offset, UNAMELEN, encoding);
		offset += UNAMELEN;
		groupName = oldStyle ? TarUtils.parseName(header, offset, GNAMELEN) : TarUtils.parseName(
				header, offset, GNAMELEN, encoding);
		offset += GNAMELEN;
		...
		switch (type) {
			...
			case FORMAT_POSIX:
			default: {
				String prefix = oldStyle ? TarUtils.parseName(header, offset, PREFIXLEN) : TarUtils
	...
	private int writeEntryHeaderField(long value, byte[] outbuf, int offset, int length,
			boolean starMode) {
