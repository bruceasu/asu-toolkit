...
public class TarOutputStream extends FilterOutputStream {
	...
	public void closeEntry() throws IOException {
		...
		if (currBytes < currSize)
			throw new IOException("entry '" + currName + "' closed at '" + currBytes
	...
	public void putNextEntry(TarEntry entry) throws IOException {
		...
		if (nameLen >= TarConstants.NAMELEN) {
			...
			if (longFileMode == LONGFILE_POSIX) {
				...
				throw new RuntimeException("file name '" + entryName + "' is too long ( > "
		...
		if (addPaxHeadersForNonAsciiNames && (entry.isLink() || entry.isSymbolicLink())
	 ...
	 * TarConstants.MAXSIZE) and other numeric values to big to fit into a
	 * traditional tar header.
	...
	public void write(byte[] wBuf, int wOffset, int numToWrite) throws IOException {
		if ((currBytes + numToWrite) > currSize)
			throw new IOException("request to write '" + numToWrite
					+ "' bytes exceeds size in header of '" + currSize + "' bytes for entry '"
					+ currName + "'");
	...
	private void addPaxHeaderForBigNumber(Map<String, String> paxHeaders, String header,
	...
	private void addPaxHeadersForBigNumbers(Map<String, String> paxHeaders, TarEntry entry) {
		...
		addPaxHeaderForBigNumber(paxHeaders, "SCHILY.devmajor", entry.getDevMajor(),
				TarConstants.MAXID);
		addPaxHeaderForBigNumber(paxHeaders, "SCHILY.devminor", entry.getDevMinor(),
				TarConstants.MAXID);
	...
	private void failForBigNumbers(TarEntry entry) {
		...
		failForBigNumber("last modification time", entry.getModTime().getTime() / 1000,
				TarConstants.MAXSIZE);
	...
	void writePaxHeaders(String entryName, Map<String, String> headers) throws IOException {
		...
		TarEntry pex = new TarEntry(name, TarConstants.LF_PAX_EXTENDED_HEADER_LC);
