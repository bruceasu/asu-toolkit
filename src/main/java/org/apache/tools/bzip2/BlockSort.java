 ...
 * M. and Wheeler, D.: A Block-sorting Lossless Data Compression Algorithm</a></li>
 ...
 * >Bentley, J.L. and Sedgewick, R.: Fast Algorithms for Sorting and Searching
 * Strings</a></li>
 * </ul>
 * 
 * @NotThreadSafe
...
class BlockSort {
	 ...
	 * &lt;= 20.
	 */
	private static final int[] INCS = { 1, 4, 13, 40, 121, 364, 1093, 3280, 9841, 29524, 88573,
	...
	/**
	 * @param fmap
	 *            points to the index of the starting point of a
	 *            permutation inside the block of data in the current
	 *            partially sorted order
	 * @param eclass
	 *            points from the index of a character inside the
	 ...
	 */
	private void mainQSort3(final CBZip2OutputStream.Data dataShadow, final int loSt,
			final int hiSt, final int dSt, final int last) {
		...
		for (int sp = 1; --sp >= 0;) {
			...
			if ((hi - lo < SMALL_THRESH) || (d > DEPTH_THRESH)) {
				...
				final int d1 = d + 1;
				final int med = med3(block[fmap[lo] + d1], block[fmap[hi] + d1],
	...
	private boolean mainSimpleSort(final CBZip2OutputStream.Data dataShadow, final int lo,
			final int hi, final int d, final int lastShadow) {
	 ...
	 */
	final void fallbackSort(final CBZip2OutputStream.Data data, final int last) {
	...
	final void mainSort(final CBZip2OutputStream.Data dataShadow, final int lastShadow) {
		...
		for (int i = 0; i <= 255; i++) {
			...
			// Step 1:
			/*
			 * LBZ2: Complete the big bucket [ss] by quicksorting any unsorted
			 * small
			...
			// Step 3:
			/*
			 * LBZ2: The ss big bucket is now done. Record this fact, and update
			 * the
